...
class XCSFActionSelection:

    def __init__(self, available_actions, p_exploration):
        self.p_exploration = p_exploration
        self.available_actions = available_actions

    def get_best_action(self, match_set, sigma):
        return 0
        '''
        action_weights = list(map(lambda x: x.action_weights, match_set))
        lin_sum = sum(list(map(lambda x, y: x[0] * y.fitness, action_weights, match_set)))
        quad_sum = sum(list(map(lambda x, y: x[1] * y.fitness, action_weights, match_set)))
        if quad_sum != 0:
            zero = -0.5 * (lin_sum / quad_sum)
        else:
            zero = 0
        if not (zero >= self.available_actions[0] and zero <= self.available_actions[1]):
            # not a feasible value
            zero = self.available_actions[0]
        loc = list(map(lambda x: x.fitness * x.get_target(sigma, zero), match_set))
        best = (zero, loc)
        left = list(map(lambda x: x.fitness * x.get_target(sigma, self.available_actions[0]), match_set))
        if loc < left:
            best = self.available_actions[0], left
        right = list(map(lambda x: x.fitness * x.get_target(sigma, self.available_actions[1]), match_set))
        if best[1] < right:
            best = self.available_actions[1], right
        return best[0]
        '''

    def get_action(self, best, train_mode):
        if random.random() < self.p_exploration and train_mode:
            rand_action = random.random() * (self.available_actions[1] - self.available_actions[0]) + self.available_actions[0]
            return rand_action
        else:
            return best
