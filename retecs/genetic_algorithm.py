...
class CIGeneticAlgorithm:
    ...
    def select_offspring(self, action_set):
        ...
        for classifier in action_set:
            ...
            if fitness_sum >= choice_point:
                return classifier
        index = random.randint(0, len(action_set) - 1)
    ...
    def apply_crossover(self, classifier_1, classifier_2):
        ...
        for i in range(x, y):
            ...
            if isinstance(classifier_1, XCSF_Classifier):
                weight_temp = classifier_1.state_weights[i + 2]
                classifier_1.state_weights[i + 2] = classifier_2.state_weights[i + 2]
                classifier_2.state_weights[i + 2] = weight_temp
        ...
        if isinstance(classifier_1, XCSF_Classifier):
            for i in range(0, len(classifier_1.action_weights)):
                a = classifier_1.action_weights[i]
                b = classifier_2.action_weights[i]
                classifier_1.action_weights[i] = alpha * a + (1 - alpha) * b
                classifier_2.action_weights[i] = (1 - alpha) * a + alpha * b
            for i in range(0, 2):
                # for duration and last execution
                a = classifier_1.state_weights[i]
                b = classifier_2.state_weights[i]
                classifier_1.state_weights[i] = alpha * a + (1 - alpha) * b
                classifier_2.state_weights[i] = (1 - alpha) * a + alpha * b
            # for the intercept
            a = classifier_1.intercept_weight
            b = classifier_2.intercept_weight
            classifier_1.intercept_weight = alpha * a + (1 - alpha) * b
            classifier_2.intercept_weight = (1 - alpha) * a + alpha * b
    ...
    def mutation(self, classifier, situation):
        ...
        if not isinstance(classifier, XCSF_Classifier):
            if random.random() < CIGeneticAlgorithm.MU:
                index = int(random.random() * len(self.possible_actions))
                classifier.action = self.possible_actions[index]
        if isinstance(classifier, XCSF_Classifier):
            for i in range(0, len(classifier.action_weights)):
                if random.random() < CIGeneticAlgorithm.MU:
                    classifier.action_weights[i] = XCSF_Classifier.get_rand_weight()
            for i in range(0, len(classifier.state_weights)):
                if random.random() < CIGeneticAlgorithm.MU:
                    classifier.state_weights[i] = XCSF_Classifier.get_rand_weight()
            if random.random() < CIGeneticAlgorithm.MU:
                classifier.intercept_weight = XCSF_Classifier.get_rand_weight()
