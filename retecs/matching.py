...
class XCSFMatching(Matching):
    ...
    def get_match_set(self, population, sigma, timestamp):
        ...
        while len(match_set) == 0:
            ...
            # ensure match set contains enough different classifiers
            if len(match_set) < self.theta_mna:
                for _ in range(0, self.theta_mna - len(match_set)):
                    # covering
                    new_classifier = self.generate_covering_classifier(sigma, [], timestamp)
                    match_set.append(new_classifier.action)
    ...
    def generate_covering_classifier(self, sigma, missing_actions, timestamp):
        ...
        # weights init
        action_weights = list(map(lambda x: XCSF_Classifier.get_rand_weight(), range(0, 2)))
        state_dim = 2 + len(sigma["previous_results"])
        state_weights = list(map(lambda x: XCSF_Classifier.get_rand_weight(), range(0, state_dim)))
        classifier = XCSF_Classifier(previous_results_conditions, last_executions_condition, duration_condition, action, timestamp, state_weights, action_weights, XCSF_Classifier.get_rand_weight())
        return classifier
