...
class Reinforcement:
    ...
    def reinforce_xcsf(self, action_set, discounted_reward, sigma, chosen_action):
        '''
        for XCSF wont work anyway
        '''
        # get L2 Norm
        l2_norm = 0 # chosen_action ** 2 + chosen_action ** 4
        l2_norm += 1
        for key in sigma.keys():
            if isinstance(sigma[key], list):
                for ele in sigma[key]:
                    l2_norm += sigma[key][ele] ** 2
            else:
                l2_norm += sigma[key] ** 2
        delta_factor = (Reinforcement.GRAD / l2_norm)
        # update using delta rule
        for classifier in action_set:
            output = classifier.get_target(sigma, chosen_action)
            update_factor = delta_factor * (discounted_reward - output)
            classifier.update_weights(update_factor, sigma, chosen_action)
        # analog stuff to XCS
        number_classifiers = sum(list(map(lambda x: x.numerosity, action_set)))
        for classifier in action_set:
            classifier.experience += 1
            # update prediction wont be used for XCSF but keep it for lazyness
            ...
            target = classifier.get_target(sigma, chosen_action)
            absolute_error = abs(discounted_reward - target)
            if classifier.experience < (1.0 / Reinforcement.BETA):
    ...
    def update_fitness(self, action_set):
        ...
        for classifier in action_set:
            ...
            if  accuracy_vector[classifier] < 0:
                pass
            else:
